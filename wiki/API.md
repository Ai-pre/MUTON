...
The current backend is implemented in `src/server_hyperclovax.py`.
...
- The image is cached as the latest face image for the next multimodal summary step.
...
- `prosody`, `content`, and `speaker` are placeholders kept for app compatibility.

## `POST /get_fusion_analysis`

### Description

Generates a multimodal Korean summary using HyperCLOVA and the committed utterance snapshot:
...
```json
{
  ...
  "arousal": 0.0,
  "valence": 0.0,
  "summary": "미간을 찌푸린 채 짜증 섞인 어조로 상황의 불편함을 드러낸다.",
  "cls_attn": []
}
```
...
- HyperCLOVA used only for multimodal summary generation
